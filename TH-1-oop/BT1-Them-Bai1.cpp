...
class Diem
{
    private:
        double x,y;
    public:
        Diem(const double a = 0, double b = 0): x(a), y(b){}
        ~Diem(){
            cout<<"HUY DIEM\n";
        }
        void Nhap(){
        ...
        bool operator==(const Diem &k ){
            return (x==k.x && y==k.y);
        }
        Diem& operator=(const Diem&k){
            x = k.x;
            y = k.y;
            return *this;
        }
        ...
        void pl(Diem yy, Diem zz) {
            ...
            if(c1==c2 && c2==c3){
                ...
                if(abs(c1*c1+c2*c2-c3*c3)<=expshi || abs(c1*c1+c3*c3-c2*c2)<=expshi || abs(c3*c3+c2*c2-c1*c2)<=expshi) cout<<"TAM GIAC VUONG CAN\n";
                else cout<<"TAM GIAC CAN"<<endl;
                return;
            } else if(abs(c1*c1+c2*c2-c3*c3)<=expshi || abs(c1*c1+c3*c3-c2*c2)<=expshi || abs(c3*c3+c2*c2-c1*c2)<=expshi){
                cout<<"TAM GIAC VUONG\n";
                return;
            } else if(!(*this==yy) && !(*this==zz) && !(yy==zz)){
